...
```
dist/
├── statusbar.css   # 全部样式（含浅色马卡龙覆写区 + 跑团判定 mx-chk 样式）
├── statusbar.js    # 主模块：开场/创建界面 + 主状态栏 + 商城 + 战斗面板
├── teammates.js    # 队友管理器模块（tm-*）
└── check.js        # 跑团判定模块（非战斗 d100 对抗 / 命运点 / 掷骰动画 / 选项联动）
...
- v1.2：非战斗跑团判定系统
  - d100 + 判定力(六维属性) 对抗 难度线(60+(档位-1)×10，B/A×1.25、S×1.5)
  - 六档结果(大成功~大失败)，神骰(≤5)/灾骰(≥96)覆盖
  - 命运点经济：大失败+1(上限3)，可重骰/提档
  - 掷骰动画 + 判定日志 + 手气统计
  - 输入区快捷骰 chips + 独立"判定"子页面 + 选项 `[判定:技能|难度]` 联动 + submitMxAction 自动包装
  - 配套世界书条目（用户自行维护）：见 statusbar-work/世界书条目-跑团判定.txt
- v1.1：修复分组开合存储反转 bug
